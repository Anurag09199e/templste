...
export const B2Course: React.FC = () => {
  const course = COURSES.find((c) => c.level === 'B2') || COURSES[3];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center mb-16">
          <div className="lg:col-span-7 space-y-6">
            <div className="flex items-center gap-3">
              <LevelBadge level="B2" size="lg" />
              <span className="text-xs font-extrabold uppercase px-3 py-1 bg-indigo-500/10 text-indigo-500 rounded-lg border border-indigo-500/20">
                {course.germanTitle}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-4 text-xs font-bold text-slate-700 dark:text-slate-300">
              ...
              <span className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 px-3 py-2 rounded-xl">
                <ShieldCheck className="w-4 h-4 text-emerald-500" /> Medical & University Standard
              </span>
            </div>

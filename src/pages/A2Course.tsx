...
export const A2Course: React.FC = () => {
  const course = COURSES.find((c) => c.level === 'A2') || COURSES[1];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center mb-16">
          <div className="lg:col-span-7 space-y-6">
            <div className="flex items-center gap-3">
              <LevelBadge level="A2" size="lg" />
              <span className="text-xs font-extrabold uppercase px-3 py-1 bg-teal-500/10 text-teal-500 rounded-lg border border-teal-500/20">
                {course.germanTitle}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-4 text-xs font-bold text-slate-700 dark:text-slate-300">
              ...
              <span className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 px-3 py-2 rounded-xl">
                <ShieldCheck className="w-4 h-4 text-emerald-500" /> Goethe A2 Exam Training
              </span>
            </div>
          ...
          <div className="lg:col-span-5">
            <GlassCard className="p-6 border border-teal-500/30">
              <img
                src={course.image}
                alt={course.title}
            ...
            {course.syllabusModules.map((mod) => (
              <GlassCard key={mod.moduleNumber} className="p-6 border border-slate-200/80 dark:border-slate-800">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-8 h-8 rounded-xl bg-teal-500 text-white font-bold flex items-center justify-center">
                    {mod.moduleNumber}
                  </div>
